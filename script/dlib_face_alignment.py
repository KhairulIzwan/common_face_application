...
class FaceAlignment_node:
	def __init__(self):
		# Initializing your ROS Node
		rospy.init_node('FaceAlignment_node', anonymous=True)
		...
		self.cv_window_name_align = "Facial Aligned"
	...
	def dispImage(self):
		...
		cv2.imshow(self.cv_window_name_align, self.faceAligned)
		cv2.waitKey(1)
	...
	def detectFacialLandmark(self):
		...
		for (i, rect) in enumerate(rects):
			...
			faceOrig = imutils.resize(self.cv_image[y:y + h, x:x + w], width=256)
			self.faceAligned = self.fa.align(self.cv_image, gray, rect)

			# show the face number
			cv2.putText(self.cv_image, "Face #{}".format(i + 1), (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
def main(args):
	vn = FaceAlignment_node()

	try:
		rospy.spin()
	except KeyboardInterrupt:
		rospy.loginfo("[INFO] FaceAlignment_node [OFFLINE]...")

	cv2.destroyAllWindows()

if __name__ == '__main__':
	rospy.loginfo("[INFO] FaceAlignment_node [ONLINE]...")
	main(sys.argv)
