...
################################################################################
## {Description}: 
## Drowsiness detection in OpenCV with ROS
################################################################################
...
################################################################################
## {Disclaimer}: 
## This code were revised from https://www.pyimagesearch.com/2017/05/08/drowsiness-detection-opencv/
################################################################################
...
class Drowsiness_node:
	def __init__(self):
		# Initializing your ROS Node
		rospy.init_node('Drowsiness_node', anonymous=True)
		...
		self.soundDir = os.path.join(self.p, "sound")
		self.sound_filename = self.soundDir + "/alarm.wav"
	...
	def textInfo(self):
		...
		cv2.putText(self.cv_image, "(%d, %d)" % (self.camerainfo.width, self.camerainfo.height), 		
			(self.camerainfo.width-100, self.camerainfo.height-10), 
			cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA, False)
					...
					if not self.ALARM_ON:
						...
						if self.sound_filename != "":
							t = Thread(target=sound_alarm,
								args=(self.sound_filename,))
							t.deamon = True
							t.start()

					# draw an alarm on the frame
					cv2.putText(self.cv_image, "DROWSINESS ALERT!", (10, 30),
...
#			cv2.putText(self.cv_image, "Blinks: {}".format(self.TOTAL), (10, 30),
#				cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
			cv2.putText(self.cv_image, "EAR: {:.2f}".format(ear), (self.camerainfo.width-100, 30),
				cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def main(args):
	...
	try:
		...
		rospy.loginfo("[INFO] Drowsiness_node [OFFLINE]...")

	cv2.destroyAllWindows()

if __name__ == '__main__':
	rospy.loginfo("[INFO] Drowsiness_node [ONLINE]...")
	main(sys.argv)
